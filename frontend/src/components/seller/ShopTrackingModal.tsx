...
interface ShopTrackingModalProps {
  isOpen: boolean
  onClose: () => void
  trackingData: any
...
export const ShopTrackingModal: React.FC<ShopTrackingModalProps> = ({
  isOpen,
  onClose,
  trackingData,
  trackingLoading,
      ...
      <div className="bg-white rounded-3xl w-full max-w-lg shadow-2xl overflow-hidden border border-slate-100 animate-in zoom-in-95 duration-200 text-left space-y-4 p-6">
        ...
        <div className="flex items-center justify-between border-b border-slate-100 pb-3">
          <div>
            <h3 className="font-black text-slate-800 text-base flex items-center gap-2">
              <span>🚚</span> Hành Trình Vận Chuyển ZMX
            </h3>
            <p className="text-[11px] text-slate-400 font-medium mt-0.5">
              {trackingData ? `Mã vận đơn: ${trackingData.trackingNumber}` : orderId ? `Đơn hàng #${orderId}` : 'Đang tra cứu hệ thống ZMX...'}
            </p>
          </div>
          <button
            onClick={onClose}
        ...
        {trackingLoading ? (
          <div className="py-12 flex flex-col items-center justify-center space-y-3">
            ...
            <p className="text-xs text-slate-500 font-bold">Đang tra cứu dữ liệu trạm vận chuyển ZMX...</p>
          </div>
        ) : !trackingData ? (
          <div className="py-8 text-center space-y-2">
            <span className="text-3xl">📦</span>
            <p className="text-xs text-slate-600 font-bold">Đang chuẩn bị điều phối tài xế ZMX</p>
            <p className="text-[11px] text-slate-400">Đơn vị vận chuyển ZMX đang điều phối tài xế khu vực đến kho lấy hàng.</p>
          </div>
        ) : (
          <div className="space-y-4">
            ...
            <div className="space-y-4 max-h-[220px] overflow-y-auto pr-1">
              <p className="text-[10px] font-black text-slate-400 uppercase tracking-wider">Lịch Sử Vận Chuyển</p>
            ...
            <div className="pt-3 border-t border-slate-100 flex items-center justify-between">
              ...
              >
                <span>🌐</span> Mở cổng ZMX Delivery Portal ↗
              </a>
              <button
                onClick={onClose}
